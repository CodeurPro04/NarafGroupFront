...
import { applyPartnership, updatePartnership, getMyPartnership, getCurrentUser, isAuthenticated } from '../api/axios';
...
const Partnership = () => {
  ...
  const isCompany = user?.role === 'entreprise' || user?.role_name === 'entreprise';
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isCompany) {
      setError("Votre compte doit etre de type entreprise pour soumettre une demande.");
      return;
    }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-2xl shadow-md p-6 md:p-10">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-semibold text-gray-900">Devenir partenaire</h1>
              <p className="text-sm text-gray-500 mt-2">
                Partagez vos informations pour construire un partenariat durable avec Naraf.
              </p>
            </div>
            {application?.status && (
              <span className="inline-flex px-3 py-1 text-xs rounded-full bg-blue-50 text-blue-600">
            ...
            <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-lg text-sm text-blue-700">
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-white border border-blue-200 text-blue-600 rounded-md text-sm"
                >
                  Creer un compte
                </button>
              </div>
          ...
          {authenticated && !isCompany && (
            <div className="mt-6 p-4 bg-amber-50 border border-amber-100 rounded-lg text-sm text-amber-700">
              Votre compte doit etre de type entreprise pour deposer une candidature.
            </div>
          )}
          ...
          {success && (
            <div className="mt-6 p-4 bg-emerald-50 border border-emerald-100 rounded-lg text-sm text-emerald-600">
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              ...
              <input
                ...
                className="mt-2 w-full rounded-md border border-gray-200 px-4 py-3 text-sm"
                required
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Numero d'enregistrement</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Numero fiscal</label>
              <input
                type="text"
                name="tax_number"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700">Services proposes (un par ligne)</label>
              <textarea
                name="services"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700">Certifications (un par ligne)</label>
              <textarea
                name="certifications"
            ...
            <div className="md:col-span-2 flex justify-end">
              <button
                ...
                className="px-6 py-3 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 disabled:opacity-60"
              >
                {saving ? 'Envoi en cours...' : application ? 'Mettre a jour' : 'Envoyer la demande'}
              </button>
            </div>
          </form>
        </div>

...
const Home = () => {
  ...
  const [partnersCanScrollLeft, setPartnersCanScrollLeft] = useState(false);
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      setPartnersCanScrollLeft(el.scrollLeft > 4);
      setPartnersCanScrollRight(maxScrollLeft - el.scrollLeft > 4);
    };
  ...
  const scrollPartnersByCard = (direction) => {
    const el = partnersCarouselRef.current;
    if (!el) return;

    const firstCard = el.querySelector("[data-partner-card]");
          ...
          setShowcaseSections([
            {
              ...
              button_label: "Voir tous les biens",
              button_link: "/properties",
              items: pickRandomItems(propertiesList, 4).map(
  ...
  return (
    <div className="bg-white">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {houseModelsLoading ? (
            ...
            <EmptyState title="Aucun modèle de maison disponible pour le moment." />
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-10 items-center bg-slate-50 border border-slate-200 p-5 sm:p-8">
              <div>
                  ...
                  {showcaseSectionsLoading ? (
                    ...
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                      {items.map((item, itemIndex) => {
                        const imageUrl =
                          toMediaUrl(item.image_url) || item.image_url;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-5xl">
            ...
            {activeVideo ? (
              ...
              <EmptyState title="Aucune video disponible pour le moment." />
            )}
          </div>
        </div>
      </section>
      ...
      {/* Section 10 — Nos Guides */}
      <GuidesSection />

      {/* Section 11 — Nos Agents */}
      <AgentsSection />

      {/* Section 12 — ABI en chiffres */}
      <StatsSection />

      {/* Section 13 — Projets neufs en exclusivité */}
      <NewProjectsSection />
      {/* Partners Section */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              Nous collaborons avec des acteurs fiables, agrées et engagés pour
              la qualité.
            </p>
          </div>
          ...
          {partnersLoading ? (
            <div className="relative">
              <div className="flex gap-6 overflow-x-auto scroll-smooth pb-4 px-1 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {[0, 1, 2, 3, 4].map((item) => (
                  <div
                    key={`partner-skeleton-${item}`}
                    className="snap-start min-w-[280px] sm:min-w-[320px] lg:min-w-[340px] bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.06)]"
                  >
            ...
            <EmptyState title="Aucun partenaire publie pour le moment." />
          ) : (
            <div className="relative">
              <div
                className={`pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-slate-50 to-transparent transition-opacity ${
                  partnersCanScrollLeft ? "opacity-100" : "opacity-0"
                }`}
              />
              <div
                className={`pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-slate-50 to-transparent transition-opacity ${
                  partnersCanScrollRight ? "opacity-100" : "opacity-0"
                }`}
              />

              <button
                type="button"
                onClick={() => scrollPartnersByCard(-1)}
                disabled={!partnersCanScrollLeft}
                className={`hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 items-center justify-center w-11 h-11 bg-white border border-slate-200 shadow-md transition ${
                  partnersCanScrollLeft
              ...
              <button
                ...
                className={`hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 items-center justify-center w-11 h-11 bg-white border border-slate-200 shadow-md transition ${
                  partnersCanScrollRight
                    ? "hover:bg-slate-50 hover:border-slate-300"
                    : "opacity-40 cursor-not-allowed"
                }`}
                aria-label="Suivant"
              ...
              <div
                ...
                className="flex gap-6 overflow-x-auto scroll-smooth pb-4 px-1 snap-x snap-mandatory [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
              >
                {partners.map((partner) => {
                  ...
                  return (
                    <Link
                      ...
                      data-partner-card
                      className="snap-start min-w-[280px] sm:min-w-[320px] lg:min-w-[340px] group block bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(15,23,42,0.1)]"
                    >
              ...
              <p className="mt-3 text-xs text-slate-500 md:hidden">
                Faites glisser horizontalement pour voir plus de partenaires.
              </p>
            </div>
          )}
        </div>

...
const PropertyDetails = () => {
  ...
  const getPropertyVisuals = () => {
    ...
    if (property.primary_image?.file_path) {
      standard.push(toMediaUrl(property.primary_image.file_path));
    }

    if (property.media && property.media.length > 0) {
      property.media.forEach((media) => {
        if (!media.file_path) return;
        const resolved = toMediaUrl(media.file_path);
        if (media.category === 'plan') {
          plan ||= resolved;
          return;
        }
        if (media.category === 'three_d') {
          render3D ||= resolved;
          return;
        }
        standard.push(resolved);
      });
    }

    const dedupedStandard = Array.from(new Set(standard.filter(Boolean)));
    if (dedupedStandard.length === 0) {
      dedupedStandard.push(
        "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=1200&q=80"
      );
    }

    return {
      standard: dedupedStandard,
      plan,
      render3D
    };
  };
  ...
  const nextImage = () => {
    const propertyVisuals = getPropertyVisuals();
    const images = propertyVisuals.standard;
 ...
 {property.transaction_type === "vente" ? "À Vendre" : "À Louer"}
        ...
        <button
          ...
          className="p-3 bg-white/90 backdrop-blur-sm hover:bg-white transition-all hover:scale-110 shadow-lg"
 ...
 <Share2 size={20} className="text-gray-600" />
        ...
        planImage={propertyVisuals.plan}
        render3DImage={propertyVisuals.render3D} />
 ...
 {/* Schedule Visit 
              <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-xl p-6">
 ...
 {/* Agent Info 
              ...
              <div className="flex items-center space-x-3">
              <div className="w-16 h-16 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center">
 ...
 {/* Security Badge 
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-xl p-6">
 ...
 <AccountCredentialsModal
        ...
        description="Conservez ce mot de passe temporaire pour suivre vos demandes et vous reconnecter à votre espace visiteur." />

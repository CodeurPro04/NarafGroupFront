  ...
  useEffect(() => {
    ...
    const loadModel = async () => {
      try {
        ...
        if (isMounted) {
          setError("Modele introuvable ou indisponible.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {!error && !loading && model && (
            <div className="space-y-8">
                ...
                <div className="space-y-5">
                  ...
                  <div className="space-y-4 text-slate-700 text-lg leading-relaxed">
                    {paragraphs.length > 0 ? (
                      ...
                      <p>Aucune description detaillee disponible.</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {gallery.length > 0 && (
                <div className="bg-white border border-slate-200 p-4 sm:p-6">
                  <h3 className="text-2xl font-bold text-slate-900 mb-4">Galerie du modele</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
